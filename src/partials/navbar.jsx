...
import { Sun, Moon } from "lucide-react"; // tambah Sun, Moon
      ...
      >
        ...
        >
            ...
            <NavbarBrand as="li" className="gap-3 max-w-fit">
              <Link className="flex justify-start items-center gap-3 group" href="/">
                ...
                <div className="flex flex-col leading-none">
                  <span className="font-black text-white text-2xl tracking-tighter group-hover:text-cyan-400 transition-colors">SPAY</span>
                  <span className="text-[10px] font-bold text-white/30 tracking-[0.2em] uppercase">SYSTEMS</span>
                </div>
              </Link>
            </NavbarBrand>
          </NavbarContent>
          ...
          <NavbarContent className="hidden lg:flex gap-12 font-bold" justify="center">
            {menuItems.map((item) => (
              <NavbarItem key={item.href}>
                <Link
                  className="text-white/40 hover:text-white transition-all text-[13px] tracking-[0.2em] uppercase relative group py-2"
                  href={item.href}
                >
                  {item.label}
          ...
          <NavbarContent className="basis-1/4 hidden lg:flex" justify="end">
            <button
              onClick={toggle}
              className="w-10 h-10 rounded-2xl bg-white/5 hover:bg-white/10 flex items-center justify-center transition-all text-white/60 hover:text-white"
          ...
          <NavbarContent className="lg:hidden basis-1 gap-2" justify="end">
            ...
            <button
              ...
              className="text-white w-12 h-12 bg-white/5 hover:bg-white/10 rounded-2xl transition-all flex items-center justify-center"
            >
              <div className="flex flex-col gap-1.5 w-5 items-center justify-center">
                <span className={`block h-[2px] bg-white rounded-full transition-all duration-300 origin-center ${isMenuOpen ? "w-5 rotate-45 translate-y-[7px]" : "w-5"}`} />
                <span className={`block h-[2px] bg-cyan-400 rounded-full transition-all duration-300 ${isMenuOpen ? "w-0 opacity-0" : "w-3.5"}`} />
                <span className={`block h-[2px] bg-white rounded-full transition-all duration-300 origin-center ${isMenuOpen ? "w-5 -rotate-45 -translate-y-[7px]" : "w-5"}`} />
              </div>
            </button>
          </NavbarContent>
          <NavbarMenu className="bg-[#020617]/80 backdrop-blur-[40px] pt-16 px-10 border-t border-white/5 mt-6 rounded-[40px] h-[calc(100vh-120px)] shadow-2xl overflow-hidden">
            <motion.div 
            ...
            >
              <div className="flex flex-col gap-8">
                {menuItems.map((item, index) => (
                  <NavbarMenuItem key={`${item.label}-${index}`}>
                    <Link
                      className="w-full text-5xl font-black text-white/10 hover:text-cyan-400 transition-all duration-500 flex justify-between items-center group decoration-none"
                      href={item.href}
              ...
              <div className="mt-auto pb-20 border-t border-white/5 pt-10 flex flex-col gap-4">
                 <p className="text-white/20 text-[10px] font-black tracking-[0.4em] uppercase">SYSTEM STATUS</p>
                 <div className="flex items-center gap-2">
                    <span className="w-2 h-2 bg-green-500 rounded-full animate-pulse shadow-[0_0_10px_rgba(34,197,94,0.5)]" />
                    <p className="text-white/60 text-xs font-bold uppercase tracking-widest">All protocols operational</p>
                 </div>
              </div>
            </motion.div>
          </NavbarMenu>
